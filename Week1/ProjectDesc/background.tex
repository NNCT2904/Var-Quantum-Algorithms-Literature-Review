...
\todo{Open with a general statement about QCs}
Quantum computing is a relatively new and emerging field in Physics, Mathematics, and Computer Science. 
\almarginpar{Need references for this}\emph{Algorithms for Quantum computers have been developed to solve problems that belong to the non-deterministic polynomial time complexity class for Classical computers}. 
...
Moreover, quantum circuits hard-code their data by design. In more detail, \almarginpar{This is an incorrect statement. the previous statement about hard coding data is correct!}\emph{the prepared state for Qubits is static as either 0s or 1s. 
...
\todo{Discuss hybrid solutions}
There are hybrid approaches involving Classical and Quantum \almarginpar{"approaches"?}\underline{hardware} to address those constraints using Machine Learning theories. 
\almarginpar{We need references for these}\emph{Variational Quantum Algorithms (VQAs) can produce Quantum circuits that receive trainable parameters, which are reusable for Quantum computers. 
...
Quantum Neural Networks (QNNs) \cite{altaisky2001quantum} \almarginpar{Avoid emphatic statements}\underline{is an exciting and} a promising paradigm involving Quantum computing and Machine learning. 
...
However, they share the same three conditions as pointed out by \almarginpar{Always refer to athors' names}\underline{author names} \cite{schuldQuestQuantumNeural2014}: 
...
\todo{Briefly discuss different QNNs}
\almarginpar{I do not understand - rephrase}Some known types of QNN \underline{in the recent quantum stage} are: 
...
Quantum Recurrent Neural Network (QRNN) is constructed as a parameterized circuit \cite{takakiLearningTemporalData2021}, with some \almarginpar{"qubits" in lower case}\underline{Qubits} being initialized and measured at each step while others memorize the past data.
However, it is still an open question whether this \almarginpar{No need to capitalise "Quantum" throughout}\underline{Quantum} alternative is better than the classical Recurrent Neural Network.
The NISQ \almarginpar{I do not understand}\underline{Quantum processor stage} also enables some emerging models such as 
...
Quantum Generative Adversarial Network \cite{dallaire-demersQuantumGenerativeAdversarial2018}\cite{lloydQuantumGenerativeAdversarial2018}\almarginpar{What would these achieve? Benefits?}.

As VQA is the mainstream method for designing QNN circuits, QNN inevitably inherited its \almarginpar{What does it mean?}\underline{parent class's} shortcomings.
One of which is the \almarginpar{"training difficulties, known as ..."}\emph{difficulty of Barren Plateaus}, as pointed out \almarginpar{\underline{by} authors}\underline{in} \cite{abbasPowerQuantumNeural2021}. \almarginpar{You have not explained anything about barren plateaus, not evident that you need to investigate further}\emph{However, the article left this problem for further study. Thus, the Barren Plateaus of QNN design under VQA is worth investigating.}
