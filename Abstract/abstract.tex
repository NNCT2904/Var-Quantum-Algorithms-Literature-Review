\begin{abstract}
    ...
    QNNs, however, have their own trainability issues, which can manifest in large variational QNN circuits, either due to their depth, a large number of qubits, or poor initialisation of circuit parameters. One such problem is the emergence of large flat areas in the loss function landscape, called \emph{barren plateaus}, which impede effective circuit optimisation.
    The methods of preventing the formation of barren plateaus or mitigating their presence in circuit optimisation have been proposed, and their variants are vigorously pursued. However, the efficacy of each method in the context of the QNN architecture and training data remains an open question.
    ...
    Three different approaches are selected to deal with barren plateaus and are then evaluated against different VQA quantum circuit structures (depth, qubits and initialisation) and a random gradient landscape.
    \\\\
